...
async function loadWatched() {
    ...
    try {
        ...
        container.innerHTML = "<p style='text-align:center; color:#999;'>Couldn't load the list right now. Try refreshing.</p>";
        return;
    }
    ...
    if (items.length === 0) {
        container.innerHTML = "<p style='text-align:center; color:#888;'>Nothing marked as watched yet!</p>";
        knownCategories = [];
        return;
    }
    ...
    for (const category in grouped) {
        ...
        grouped[category].forEach(item => {
            ...
            card.innerHTML = `
                ...
                <div class="overlay">${item.anime_title}</div>
            `;
            ...
            if (adminMode) {
                ...
                deleteBtn.style.cssText = "position:absolute; top:6px; right:8px; z-index:5; cursor:pointer; background:rgba(255,255,255,0.85); border-radius:50%; width:26px; height:26px; display:flex; align-items:center; justify-content:center; font-size:0.9em;";
                deleteBtn.onclick = (e) => {
                    e.stopPropagation();
...
function requestRemove(id, title) {
    currentId = id;
    currentTitle = title;
    document.getElementById('removeText').innerText = `Are you sure you want to remove "${title}" from Watched?`;
...
if (searchBtn) {
    ...
    document.getElementById('watchedSearchInput').addEventListener('keydown', (e) => {
...
async function searchForWatchedAdd() {
    ...
    try {
        ...
        results.forEach(anime => {
            ...
            `;
            card.onclick = () => {
                pendingAddAnime = { title, image: anime.coverImage.large, link: anime.siteUrl };
                openCategoryModal();
            };
